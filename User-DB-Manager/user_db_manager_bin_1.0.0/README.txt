...
/------------------------Installation procedure--------------------------------/
 ...
 1.	Extract the user_db_manager_bin_1.0.0.tar.gz file.
 2.	Open the file user_db_mgr_config.conf and add the path of ppm data files in this config file.
 3.	Follow the instruction given in next heading to update the user_db_mgr_config.conf file.
 4.	This config (user_db_mgr_config.conf) file is provided for the flexibility of report generation.
 5.	Run the script user_db_install.sh
 6.	Restart the system.
 7.	On system boot up user_db_manager program would be running in background.
 8.	Program running can be check with command: ps -ax | grep user_db_manager
 9.	Now run the client utility (./test_db_manager) to communicate with User and DB manager program.
